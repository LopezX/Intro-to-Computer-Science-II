 ...
 *    - Tested:
 *         - displayItemInfo
 *         - printCString (within displayItemInfo)
 *         - stuDblToCstr (within displayItemInfo)
 *         - calcProfit (within displayItemInfo)
 *
 * date modified:  2/28/2022
 *    - Tested:
 *         - stuCstrToDbl (within setting variables)
 *         - All get[variable] functions
 ...
 *
 *
 * This is the test main to test all the functions in "ItemInfo.hpp".
 */
...
int main() {
    ...
    char *ID = new char [6];
    char *man = new char [10];
    char *sell = new char [10];
    char c;
    int i, j, k, whereToStore;

    for (i = 0; i < size; i++) {
        printCString(cout, "Product ");
        cout.put(static_cast<char>(i + '1'));
        cout.put(':');
        cout.put('\n');
        products[i].displayItemInfo(cout);
        cout.put('\n');
    }
    ...
    //Displays each item's info
    for (i = 0; i < count; i++) {
        products[i].displayItemInfo(cout);
        cout.put('\n');
    }

    printCString(cout, "Get Product 1's itemId: ");
    stuDblToCstr(ID, static_cast<double>(products[0].getItemId()));
    ID[5] = '\0';
    printCString(cout, ID);
    cout.put('\n');

    printCString(cout, "Get Product 2's description: ");
    printCString(cout, products[0].getDescription());
    cout.put('\n');

    printCString(cout, "Get Product 2's manCost: ");
    stuDblToCstr(man, static_cast<double>(products[0].getManCost()));
    printCString(cout, man);
    cout.put('\n');

    printCString(cout, "Get Product 1's sellPrice: ");
    stuDblToCstr(sell, static_cast<double>(products[0].getSellPrice()));
    printCString(cout, sell);
    cout.put('\n');
    cout.put('\n');
